...
class _DiceRollPopupState extends State<DiceRollPopup>
  ...
  Future<int> rollDamageDice(String damageDice,
      {bool isCrit = false}) async {
    final match =
  ...
  String formatRolls(List<int> rolls, int modifier) {
    final String rollsString = rolls.join(', ');
    final int total = rolls.reduce((a, b) => a + b) + modifier;
    return "Rolls: $rollsString + $modifier\nTotal: $total";
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (isRolling) {
      ...
      content = Column(
        ...
        children: [
          Text(
            "Attack Roll: $displayedAttackTotal", // Only show the total
            style: const TextStyle(fontSize: 16),
          ),
          const SizedBox(height: 16),
          Text(
            "Damage Roll: $damageTotal", // Only show the total
            style: const TextStyle(fontSize: 16),
          ),
        ],
      );
      ...
      content = Column(
        ...
        children: [
          Text(
            widget.isAttack
                ? "Attack Roll:\n${formatRolls(diceValues, widget.modifier)}"
                : "Roll:\n${formatRolls(diceValues, widget.modifier)}",
            style: const TextStyle(fontSize: 16),
          ),
        ],
          ...
          if (widget.isAttack && !damageRolled)
            Row(
              ...
              children: [
                TextButton(
                  onPressed: () async {
                    final int damageBaseRoll = await rollDamageDice(
                      widget.attackRollDamage,
                      //check if its a crit
                      isCrit: widget.diceSides == 20 && diceValues.isNotEmpty && diceValues[0] == 20, 
                    ...
                    // Show the damage roll results
                    final String damageBreakdown = formatRolls(diceValues, widget.modifier);
                    showDialog(
                      context: context,
                      builder: (context) => AlertDialog(
                        title: const Text(
                        ...
                        actions: [
                          TextButton(
                            ...
                            child: const Text(
                              "OK",
                              style: TextStyle(fontSize: 16), // Adjust button font size
                            ),
                          ),
                        ],
                      ),
